        ...
        if (CC_WECHATGAME) {
            window.wx.getUserCloudStorage({
                ...
                success: function (getres) {
                    cc.log('getUserCloudStorage', 'success', getres)
                    if (getres.KVDataList.length != 0) {
                        this.score = getres.KVDataList[0].value
                        cc.log('getUserCloudStorage', 'ok')
                    }
                },
                fail: function (res) {
                    cc.log('getUserCloudStorage', 'fail')
                },
    ...
    submitScore(MAIN_MENU_NUM, score) { //提交得分
        ...
        window.wx.setUserCloudStorage({
            ...
            success: function (res) {
                cc.log('setUserCloudStorage', 'success', res)
            },
            fail: function (res) {
                cc.log('setUserCloudStorage', 'fail')
            },
            complete: function (res) {
                // cc.log('setUserCloudStorage', 'ok')
            }
        });
    },
    removeChild() {
        // this.node.removeChildByTag(1000);
        // cc.log(this.node.getChildByName("1000"))
        // this.node.getChildByName("1000").destroy()
            ...
            wx.getUserInfo({
                ...
                success: (userRes) => {
                    ...
                    wx.getGroupCloudStorage({
                        ...
                        keyList: [MAIN_MENU_NUM],
                        success: res => {
                            cc.log("wx.getGroupCloudStorage success", res);
                            this.loadingLabel.active = false;
                            let data = res.data;
                            data.sort((a, b) => {
                                if (a.KVDataList.length == 0 && b.KVDataList.length == 0) {
