 ...
 * 去掉末尾0数
 */
function dealEndZero(value) {
    var valueStr = value + "";
    if (valueStr.indexOf(".") != -1) {
        var temp0 = valueStr.split(".")[0];
        var temp1 = valueStr.split(".")[1];
        var index = -1;
        for (var t = temp1.length - 1; t >= 0; t--) {
            if (temp1.charAt(t) != "0") {
                index = t;
                break;
            }
        ...
        if (index >= 0) {
            valueStr = temp0 + "." + temp1.substring(0, index + 1);
        } else {
            valueStr = temp0;
        }
    }
 ...
 * 100W以内转换为W并有一个小数点
 */
UtilsOther.numberToUnity = (value, w = "万") => {
    if (value < 10000)
        return dealEndZero(value + "") + "";
    if (value < 100000)
        return dealEndZero((value / 10000).toFixed(2)) + w;
    if (value < 1000000)
        return dealEndZero((value / 10000).toFixed(1)) + w;
    if (value > 100000000)
        return dealEndZero((value / 100000000).toFixed(2)) + "亿";
    return dealEndZero((Math.floor(value / 10000)).toFixed()) + w;
}
